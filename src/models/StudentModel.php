...
class StudentModel extends DB{
    ...
    public function searchstudent($search){
      return $this->select("students", " branches.name as branches_name,students.name as students_name,email,password,phone,students.id ")->join("inner", "branches", "students.branch_id", "branches.id")->where("students`.`name","like", '%'.$search.'%')->orwhere("branches`.`name", "like", '%'.$search.'%')->orwhere("phone", "like", '%'.$search.'%')->all();
    }
